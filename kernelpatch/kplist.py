...
class _KPlistBuilder(object):
    ...
    def __init__(self):
        ...
        self.tags = set(self.start_handler.keys()).union(
            self.end_handler.keys())
    ...
    def start(self, tag, attr):
        ...
        for attrname in set(attr.keys()).difference(('ID', 'IDREF')):
            if attrname not in self.attributes[tag]:
                raise ValueError(
                    'illegal attribute "{}" for tag "{}"'.format(attrname, tag))
    ...
    def add_object(self, tag, value):
        ...
        if tag == 'key':
            ...
            if not self.collection_stack or type(self.collection_stack[-1]) is not dict:
            ...
            assert type(self.collection_stack[-1]) is dict
            if self.current_key in self.collection_stack[-1]:
                raise ValueError(
                    'duplicate key "{}" in dict'.format(self.current_key))
            self.collection_stack[-1][self.current_key] = value
            self.current_key = None
        elif self.root is None:
            # We are setting the root object.
            self.root = value
        elif self.collection_stack and type(self.collection_stack[-1]) is list:
